...
glm::vec3 cubePositions[] = {
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    // load image, create texture and generate mipmaps
    data = stbi_load("images/co.png", &width, &height, &nrChannels, 0);
    if (data)
    {
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glUniformMatrix4fv(viewLocation, 1, GL_FALSE, glm::value_ptr(view_matrix));

        int projectionLocation = glGetUniformLocation(ourShader.ID, "projection");
        glUniformMatrix4fv(projectionLocation, 1, GL_FALSE, glm::value_ptr(projection_matrix));
        ...
        for (int i = 0; i < 10; i++){
            ...
            if (i % 3 == 0 || i == 0) {
                model_matrix = glm::rotate(model_matrix, (float)sin(glm::radians(angle) * glfwGetTime()), glm::vec3(1.0f, 0.3f, 0.5f));
            }
            else {
                model_matrix = glm::rotate(model_matrix, glm::radians(angle), glm::vec3(1.0f, 0.3f, 0.5f));
            }
            glUniformMatrix4fv(modelLocation, 1, GL_FALSE, &model_matrix[0][0]);
